...
class OrganizzazioneControl {
    ...
    async deleteReportORG(reportID){
        const reportDAO = new ReportORG_DAO();

        await reportDAO.removeByID(reportID);
    }

    async updateReportInfo(reportID, name, description){
        const reportDAO = new ReportORG_DAO();

        await reportDAO.updateLastReport(reportID, name, description);
    }
}
